...
#include "GradDamageExecution.h"
#include "AbilitySystem/Attributes/GradHealthSet.h"
#include "AbilitySystem/Attributes/GradCombatSet.h"
//#include "AbilitySystem/GradGameplayEffectContext.h"
//#include "AbilitySystem/GradAbilitySourceInterface.h"
#include "Engine/World.h"
//#include "Teams/GradTeamSubsystem.h"

#include UE_INLINE_GENERATED_CPP_BY_NAME(GradDamageExecution)
...
struct FDamageStatics
{
	...
	FDamageStatics()
	{
		BaseDamageDef = FGameplayEffectAttributeCaptureDefinition(UGradCombatSet::GetBaseDamageAttribute(), EGameplayEffectAttributeCaptureSource::Source, true);
	}
};
...
void UGradDamageExecution::Execute_Implementation(const FGameplayEffectCustomExecutionParameters& ExecutionParams, FGameplayEffectCustomExecutionOutput& OutExecutionOutput) const
{
#if WITH_SERVER_CODE
	//const FGameplayEffectSpec& Spec = ExecutionParams.GetOwningSpec();
	//FGradGameplayEffectContext* TypedContext = FGradGameplayEffectContext::ExtractEffectContext(Spec.GetContext());
	//check(TypedContext);
	...
	//if (HitActor)
	//{
	//	UGradTeamSubsystem* TeamSubsystem = HitActor->GetWorld()->GetSubsystem<UGradTeamSubsystem>();
	...
	//if (const IGradAbilitySourceInterface* AbilitySource = TypedContext->GetAbilitySource())
	//{
	...
	const float DamageDone = 50.f;

	if (DamageDone > 0.0f)
	{
		// Apply a damage modifier, this gets turned into - health on the target
		OutExecutionOutput.AddOutputModifier(FGameplayModifierEvaluatedData(UGradHealthSet::GetDamageAttribute(), EGameplayModOp::Additive, DamageDone));
	}
