...
void UGradCameraMode::UpdateView(float DeltaTime)
{
	...
	// FGradCameraModeView에 PivotLocation/Rotation 설정
...
{
	// 우리가 앞서 UGradCameraMode를 생성하는 곳은 UGradCameraModeStack이었다:
	// - 해당 코드를 보면, GetOuter()를 GradCameraMode로 GradCameraComponent로 설정하였다
	// - UGradCameraModeStack::GetCameraModeInstance() 확인
...
void UGradCameraModeStack::BlendStack(FGradCameraModeView& OutCameraModeView) const
{
	...
	for (int32 StackIndex = (StackSize - 2); StackIndex >= 0; --StackIndex)
	{
		...
		// GradCameraModeView Blend 함수를 구현하자:
